...
public class StringEx_02 {
	public static void main(String[] args) {
		...
		// subNation에 담아라!
		String subNation = strNation.substring(9,11); // 9 <= x < 11
		System.out.println(subNation);
		
		// 문자열 개수가 몇개인가?
		int strLength = strNation.length();
		
		// strNation 문자열을 한글자씩 잘라서 console에 보이기
		for(int i = 0; i < strLength; i++) {
			String s = strNation.substring(i, i+1);
			System.out.println(s);
		}
		
		// 문자열을 한글자씩 분해하여 '문자배열'로 변환하여 charNations에 저장하라
		...
		// 문자열을 한글자씩 분해하여 '문자열 배열'로 변환하여 strNations에 저장하라
