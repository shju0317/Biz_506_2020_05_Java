...
public class MemberEx {
	 ...
	 * 
	 * args 배열은 현재 클래스를 cmd창에서 실행할 때
	 * 'java MemberEx 문자열' 형식으로 명령을 입력하면
	 * 클래스 뒤에 따라오는 문자열이 args변수에 담겨서
	 * 진입점 main method로 전달된다. 
	 */
	public static void main(String[] args) {
		//System.out.println(args[0]);
		System.out.println("여기는 클래스 진입점 method입니다.");
		
		int num1 = 20;
		int num2 = 30;
		int sum = num1 + num2;
		System.out.println("두 수의 덧셈 결과: " + sum);
		
		/*
		 * 변수를 선언만한 상태에서 변수를 읽으려고 시도를 하면
		 * 문법 오류가 나타난다.
		 * 선언만한 상태의 변수는 어떤 값이 저장되어있는지 알 수 없는 상태이다
		 * 이 상태에서 값을 읽는 동작을 수행하게 되면
		 * 시스템 메모리에 저장된 값이 노출되는 현상이 발생을 한다
		 * 그래서 자바 초기화 되지 않은 변수에 담긴 값은 확인할 수 없도록
		 * 언어문법구조가 형성되어있다. 
		 */
		int num3 = 0;
		 ...
		 * 문자열, 클래스형 변수는 null값을 저장하는 것이 좋다.
		 */
	}
}
