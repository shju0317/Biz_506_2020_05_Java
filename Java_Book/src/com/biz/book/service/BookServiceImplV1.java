 ...
 * add implements method를 실행하여야 한다.
 */

//title;comp;writer;year;price ;
...
public class BookServiceImplV1 implements BookService{
	...
	public boolean inputBook() {
		...
		// 필드변수에 선언하지않고 method의 지역변수로 선언, 초기화를 수행
		...
		while(true) {
			System.out.print("도서명(종료:END)>> ");
			String strTitle = scan.nextLine();
			
			if(strTitle.equalsIgnoreCase("END")) {
				return false;
			}
			...
			try {
				 ...
				 */
				System.out.println("[WARNING] 출판년도는 숫자만 입력가능!");
				System.out.println(bookVO.getTitle() + "도서명부터 다시 입력하세요!");
				return true;
			}
			...
			try {
				...
				System.out.println("[WARNING] 가격은 숫자만 입력가능!");
				System.out.println(bookVO.getTitle() + "도서명부터 다시 입력하세요!");
				return true;
			}
			/*
			 ...
			 * 아직 List에 추가되기 전이므로 현재의 입력된 도서정보는 모두 무시되어 버린다.
			 * main에서는 true가 return되었으므로
			 * 아무런 판단없이 다시 입력을 수행하는 inputBook() method를 호출하게 될 것이고
			 * return하기 전에 현재 도서를 다시 입력하라는 메시지를 보여주었으므로
	...
	public void booklist() {
		...
		System.out.println("-------------------------------------------------------");
		System.out.println("책제목\t출판사\t저자\t출판년도\t가격");
		System.out.println("-------------------------------------------------------");
		
		for(BookVO bVO : bookList) {
			System.out.printf("%s\t%s\t%s\t%d\t%d\n", bVO.getTitle(), bVO.getComp(), bVO.getWriter(), bVO.getYear(), bVO.getPrice());
		}
