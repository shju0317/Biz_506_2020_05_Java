...
public class ScoreEx_03 {
	public static void main(String[] args) {
		...
		// 선언을 할 때는 List를 사용해서 선언을 하고
		// 초기화를 할때는 new ArrayList()를 사용해서 생성을 한다.
		// str1과 int1은 개수가 정해지지 않은
		// 특별한 배열과 같은 존재가 된다.
		// 'List type의 인스턴스, List type의 객체, str1리스트'라고 칭한다.
		
		// String형 데이터들을 포함할 리스트
		// size가 0인 String형 데이터들을 포함할 리스트를 생성하는 명령문
		List<String> str1 = new ArrayList<String>();
		
		//Generic : <> 가 들어가는 키워드 문법형식. 
		...
		// java 1.8 이상에서는
		// 초기화 코드(new ArrayList)에 있는 Generic을 생략할 수 있다.
		List<ScoreVO> score18 = new ArrayList<>();
		
		// 개발자가 작성한 ScoreVO클래스 type의 인스턴스를 포함할 수 있는 리스트
		List<ScoreVO> scoreList = new ArrayList<ScoreVO>();
		
		// jdk에서 제공되는 Random클래스 type의 인스턴스를
		// 포함할 수 있는 리스트
